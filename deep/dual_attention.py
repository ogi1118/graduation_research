...
class DualAttention:
    ...
    def read(self, doc, max_sentence_number=20, appy_executer=True, file_name=None, col_name_from=None, col_name_to=None):
        if appy_executer:
            if file_name is None or col_name_from  is None or col_name_to is None:
                ...
                sc = dr.SentenceConverter(file_name, col_name_from, col_name_to)
                embedding_tensor, unpadding_flags = sc.convert(max_sentence_number)
                self.sentence_lists = sc.get_sentence_lists()
        else:
            sentence_enc = se.SentenceEncoder()
            embedding_tensor, unpadding_flags = sentence_enc.doc_encode(doc, max_sentence_number=max_sentence_number)
        ...
        for e, u in zip(embedding_tensor, unpadding_flags):
            input_tensor = e[0].unsqueeze(0)
            output_tensor = e[1].unsqueeze(0)
            input_padding_flags = u[0].unsqueeze(0)
            output_padding_flags = u[1].unsqueeze(0)
            #
            doc_in_embed_array.append(input_tensor)
            doc_in_unpad_array.append(input_padding_flags)
            doc_out_embed_array.append(output_tensor)
            doc_out_unpad_array.append(output_padding_flags)
        
        doc_in_embed_tensor = torch.cat(doc_in_embed_array)
        doc_in_unpad_tensor = torch.cat(doc_in_unpad_array)
        doc_out_embed_tensor = torch.cat(doc_out_embed_array)
        doc_out_unpad_tensor = torch.cat(doc_out_unpad_array)
        self.read_flag = True
        return doc_in_embed_tensor, doc_in_unpad_tensor, doc_out_embed_tensor, doc_out_unpad_tensor
