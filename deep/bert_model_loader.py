...
class BertLoader:
    ...
    def get_CLS(self, texts, flags=None):
        ...
        if flags is not None:
            ...
            zero_vectors = torch.zeros((zero_flag_num,vector_dim)).to(self.device)
    ...
    def get_avepool(self, texts):
        ...
        sentence_embeds = torch.sum(token_embeds * input_mask_expanded, dim=1) / torch.clamp(input_mask_expanded.sum(dim=1), min=0.1)
